...
TEST_F(StoreFixture, functionDesignatorFormStoresNameOnPlan) {
    ...
    expr.setFunctionDesignatorResult(store, symbols::ValueEntry { "t0", ptr, ctx(), 0 }, "foo");
...
TEST_F(StoreFixture, takeValueFromRequiresResult) {
    ast::IdentifierExpression src { "x", ctx() };
    ast::IdentifierExpression dest { "g", ctx() };
    EXPECT_THROW(dest.takeValueFrom(src, store), std::runtime_error);
}
...
TEST_F(StoreFixture, takeValueFromCopiesScalarResultAndLvalue) {
    ...
    ASSERT_TRUE(dest.hasResult(store));
    EXPECT_EQ(dest.result(store)->getName(), "x");
    ASSERT_NE(dest.lvalueAnnotation(store), nullptr);
    EXPECT_EQ(dest.lvalueAnnotation(store)->getName(), "xp");
...
TEST_F(StoreFixture, takeValueFromCopiesAggregateFormAndAddressPlan) {
    ...
    EXPECT_EQ(dest.result(store)->getName(), "sp");
...
TEST_F(StoreFixture, takeValueFromCopiesFunctionDesignatorClearsLval) {
    ...
    src.setFunctionDesignatorResult(store,
            symbols::ValueEntry { "t", type::pointer(fn), ctx(), 0 }, "f");
...
TEST_F(StoreFixture, genericSelectionSelectAdoptsValueAndCategory) {
    ...
    ASSERT_TRUE(generic.hasResult(store));
    EXPECT_EQ(generic.result(store)->getName(), "x");
}
