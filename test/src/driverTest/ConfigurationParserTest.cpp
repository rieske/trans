...
TEST(ConfigurationParser, ignoresUnknownGccFlags) {
	...
	char *argv[] = { executable, wall, o2, fpic, std, sourceFileName };

	ConfigurationParser parser(6, argv);
	Configuration configuration = parser.getConfiguration();

	ASSERT_THAT(configuration.getSourceFiles(), ElementsAre(StrEq("a.c")));
	// Residual gcc noise must not become link args or sources.
	EXPECT_THAT(configuration.getLinkArgs(), IsEmpty());
}
