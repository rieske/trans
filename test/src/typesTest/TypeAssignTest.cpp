...
TEST(Type, productAssignFromPointerToStructUnionForSockaddrArg) {
    ...
    sockaddrArgUnion.markTransparentUnion();
    ...
    auto unmarked = type::unionType({ { "__sockaddr__", sockaddrPtr } });
    EXPECT_FALSE(type::productAssignFrom(unmarked, sockaddrPtr));
    EXPECT_TRUE(type::productAssignFrom(sockaddrPtr, unmarked));
