...
bool isZeroSpan(const symbols::StructFieldInit& f) {
    return std::holds_alternative<symbols::ZeroSpanInit>(f);
}

bool isZeroSpanAt(const symbols::StructFieldInit& f, int offset) {
    const auto* z = std::get_if<symbols::ZeroSpanInit>(&f);
    return z && z->offsetBytes == offset;
}

bool isZeroCovering(const symbols::StructFieldInit& f, int offset, int minSpan) {
    const auto* z = std::get_if<symbols::ZeroSpanInit>(&f);
    return z && z->offsetBytes == offset && z->zeroSpanBytes >= minSpan;
}

bool isConstantAt(const symbols::StructFieldInit& f, int offset, const char* value) {
    const auto* c = std::get_if<symbols::ConstantStoreInit>(&f);
...
TEST(InitializerLowering, fieldPlanDesignatedNestedStringElision) {
    ...
    bool sawFullObjectZero = false;
    for (const auto& f : fields) {
        if (const auto* s = std::get_if<symbols::StringBytesInit>(&f)) {
...
TEST(InitializerLowering, classifyNestedBracedStringIsCharArrayString) {
    ...
    EXPECT_FALSE(isCharArrayType(bools));
...
TEST(InitializerLowering, placeStringArrayRejectsBoolArray) {
    ...
    EXPECT_FALSE(isCharArrayType(boolArr));
    EXPECT_TRUE(isCharArrayType(type::array(type::signedCharacter(), 4)));
...
TEST(InitializerLowering, completeCharArrayFromStringThenPack) {
    ...
    std::vector<symbols::StructFieldInit> fields;
    lowerToFieldInits(type, str.get(), symbols, host,
            [&](symbols::StructFieldInit f) { fields.push_back(std::move(f)); });
    ASSERT_TRUE(errors.empty()) << (errors.empty() ? "" : errors.front());
    ASSERT_EQ(fields.size(), 1u);
    const auto* packed = std::get_if<symbols::StringBytesInit>(&fields[0]);
