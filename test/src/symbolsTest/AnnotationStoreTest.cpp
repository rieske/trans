...
TEST(AnnotationStore, structFieldInitsAreSideTable) {
    int node = 3;
    symbols::AnnotationStore store;
    symbols::ZeroSpanInit init;
    init.offsetBytes = 8;
    init.zeroSpanBytes = 4;
    init.temps.source = std::make_unique<symbols::ValueEntry>("s0", type::signedInteger(), ctx(), 0);
    init.temps.address = std::make_unique<symbols::ValueEntry>("a0", type::pointer(type::signedInteger()), ctx(), 0);
