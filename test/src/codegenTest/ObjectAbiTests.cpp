...
// Pure SysV: complete records only (not arrays, not incomplete).
TEST(ObjectAbi, typeNeedsMemoryReturnSysVCompleteRecordsOnly) {
    ...
    // Arrays are aggregates but pure SysV does not sret them.
    auto bigArr = type::array(type::signedCharacter(), 24);
    EXPECT_TRUE(bigArr.isAggregate());
    EXPECT_FALSE(typeNeedsMemoryReturn(bigArr));
    EXPECT_TRUE(productNeedsMemoryReturn(bigArr));
}

// Product emission uses productNeedsMemoryReturn + variadic suppress.
TEST(ObjectAbi, productEmitsMemoryReturnRespectsVariadic) {
    auto large = type::structure({
            { "a", type::signedLong() },
            { "b", type::signedLong() },
            { "c", type::signedLong() },
    });
    EXPECT_TRUE(productNeedsMemoryReturn(large));
    EXPECT_TRUE(productEmitsMemoryReturn(large, false));
    EXPECT_FALSE(productEmitsMemoryReturn(large, true));
    EXPECT_FALSE(productEmitsMemoryReturn(type::signedInteger(), true));
    EXPECT_FALSE(productEmitsMemoryReturn(type::signedInteger(), false));
}
...
TEST(ObjectAbi, incompleteRecordIsNotSret) {
    ...
    EXPECT_FALSE(productNeedsMemoryReturn(inc));
}
...
TEST(ObjectAbi, largeUnionNeedsSret) {
    ...
    EXPECT_TRUE(productNeedsMemoryReturn(u));
}
