...
TEST(FrameLayout, addressOfExtendsMultiWordObjectThroughPointerUse) {
    ...
    locals.emplace("$t1",
            symbols::ValueEntry { "$t1", arrTy, ctx(), 1});
    locals.emplace("$t2",
            symbols::ValueEntry { "$t2", ptrTy, ctx(), 2});
    locals.emplace("$t3",
            symbols::ValueEntry { "$t3", intTy, ctx(), 3});
...
TEST(FrameLayout, leaObjectFieldAddressExtendsMultiWordObjectThroughPointerUse) {
    ...
    type::Type ptrTy = type::pointer(type::signedInteger());
    type::Type intTy = type::signedInteger();
    locals.emplace("$t1", symbols::ValueEntry { "$t1", arrTy, ctx(), 1});
    locals.emplace("$t2", symbols::ValueEntry { "$t2", ptrTy, ctx(), 2});
    locals.emplace("$t3", symbols::ValueEntry { "$t3", intTy, ctx(), 3});
...
TEST(FrameLayout, paramCallExtendDoesNotShrinkPostCallPointerUse) {
    ...
    locals.emplace("$t3",
            symbols::ValueEntry { "$t3", intTy, ctx(), 3});
    locals.emplace("$t4",
            symbols::ValueEntry { "$t4", ptrTy, ctx(), 4});
...
TEST(FrameLayout, assignLabelAddressLiveAcrossNestedCall) {
    ...
    locals.emplace("$t1",
            symbols::ValueEntry { "$t1", ptrTy, ctx(), 1});
    locals.emplace("$t2",
            symbols::ValueEntry { "$t2", intTy, ctx(), 2});
    locals.emplace("$t3",
            symbols::ValueEntry { "$t3", intTy, ctx(), 3});
    locals.emplace("$t4",
            symbols::ValueEntry { "$t4", intTy, ctx(), 4});
...
TEST(FrameLayout, lastUseOrdinalsMatchBodyAfterRunIrPasses) {
    ...
    type::Type intTy = type::signedInteger();
    locals.emplace("$t1",
            symbols::ValueEntry { "$t1", intTy, ctx(), 1});
