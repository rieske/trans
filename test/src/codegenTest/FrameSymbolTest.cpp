...
TEST(FrameSymbol, mapsIntegralValueEntry) {
    ...
    symbols::ValueEntry e { "x", type::signedInteger(), ctx, 3 };
...
TEST(FrameSymbol, mapsUnsignedAndFloating) {
    ...
    symbols::ValueEntry z { "z", type::complexFloat(), ctx, 2 };
    FrameSymbol fz = frameSymbolFrom(z, true);
    EXPECT_FALSE(fz.isFloating());
    EXPECT_EQ(fz.valueKind(), codegen::ValueKind::COMPLEX);
    EXPECT_EQ(fz.sizeBytes(), 8);
}

TEST(FrameSymbol, mapsPointerAndAggregateSizes) {
    translation_unit::Context ctx { "t", 1 };
    symbols::ValueEntry p { "p", type::pointer(type::signedInteger()), ctx, 0 };
    ...
    symbols::ValueEntry s { "s", st, ctx, 0 };
...
TEST(FrameSymbol, emptyTypeGetsWordHome) {
    translation_unit::Context ctx { "t", 1 };
    symbols::ValueEntry e { "v", type::voidType(), ctx, 0 };
...
TEST(Value, defaultSixteenByteFloatIsX87) {
    codegen::Value v { "ld", 0, codegen::ValueKind::FLOATING, 16 };
    const auto c = v.getClassification();
...
TEST(Value, defaultEightByteFloatStaysSse) {
    codegen::Value v { "d", 0, codegen::ValueKind::FLOATING, 8 };
    EXPECT_FALSE(v.getClassification().hasX87());
    EXPECT_TRUE(v.getClassification().inRegisters());
    EXPECT_EQ(v.getClassification().alignBytes, 8);
}

TEST(FrameSymbol, longDoubleToValueIsX87) {
    translation_unit::Context ctx { "t", 1 };
    symbols::ValueEntry e { "ld", type::longDoubleFloating(), ctx, 0 };
    codegen::Value v = frameSymbolFrom(e).toValue();
    EXPECT_TRUE(v.getClassification().hasX87());
    EXPECT_EQ(v.getClassification().alignBytes, 16);
    EXPECT_EQ(v.getSizeInBytes(), 16);
}

TEST(FrameSymbol, toValueAtSlotOverridesEntryIndex) {
    translation_unit::Context ctx { "t", 1 };
    symbols::ValueEntry e { "tmp", type::signedLong(), ctx, 99 };
