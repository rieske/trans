...
TEST(SymbolRefs, assignLabelAddressDefsResult) {
    Instruction op = ir::assignLabelAddress("__str1", "$t6");
    SymbolRefs refs;
    collectSymbolRefs(op, refs);
    EXPECT_TRUE(refs.uses.empty());
    ASSERT_EQ(refs.defs.size(), 1u);
...
TEST(SymbolRefs, leaObjectIndexAddressReportsAddressOfBase) {
    Instruction op = ir::indexAddress("$t1", "$t2", 4, "$t3", symbols::AddressBaseMode::LeaObject);
    SymbolRefs refs;
    collectSymbolRefs(op, refs);
    EXPECT_EQ(refs.addressOfBase, "$t1");
}

TEST(SymbolRefs, pointerValueIndexAddressDoesNotReportAddressOfBase) {
    Instruction op = ir::indexAddress("$t1", "$t2", 4, "$t3", symbols::AddressBaseMode::PointerValue);
    SymbolRefs refs;
    collectSymbolRefs(op, refs);
    EXPECT_TRUE(refs.addressOfBase.empty());
}
...
TEST(SymbolRefs, assignUsesOperandDefsResult) {
    ...
    ASSERT_EQ(refs.defs.size(), 1u);
    EXPECT_EQ(refs.defs[0], "$t2");
    EXPECT_EQ(refs.assignCopyFrom, "$t1");
    EXPECT_TRUE(refs.addressOfBase.empty());
}

TEST(SymbolRefs, bswapUsesOperandDefsResult) {
    Instruction op = ir::bswap("$t1", "$t2", 4);
    SymbolRefs refs;
    collectSymbolRefs(op, refs);
    ASSERT_EQ(refs.uses.size(), 1u);
    EXPECT_EQ(refs.uses[0], "$t1");
