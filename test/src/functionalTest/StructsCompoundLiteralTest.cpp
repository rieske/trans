...
TEST(Compiler, scalarCompoundLiteralPointer) {
    SourceProgram program{R"prg(#include <stdio.h>
        int main() {
            int *p;
            p = (int *){ 0 };
...
// Compound literal with a single scalar expression (not a brace-list reduce).
TEST(Compiler, compoundLiteralSingleExpressionElement) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct S { int x; int y; };
        int main() {
            /* first element only; rest zero-filled */
...
// Trailing comma in compound literal list.
...
// Member access on a compound literal must LEA the object, not load its first
// word as a pointer.
...
// IndexAddress of a multi-word array CL must keep the array live while a later
// argument materializes another same-size CL (the reclaiming temp).
...
// FieldAddress of a multi-word struct CL must keep the object live while a
// later argument materializes another same-size CL.
...
// Compound literal with __typeof__(expr) type name resolves to the operand's type.
