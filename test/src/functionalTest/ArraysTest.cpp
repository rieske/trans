...
TEST(Compiler, incompleteLocalArrayFromBrace) {
    SourceProgram program{R"prg(
        int main() {
            int a[] = { 10, 20, 12 };
            printf("%d %d", (int)(sizeof(a) / sizeof(a[0])), a[0] + a[1] + a[2]);
            return 0;
        }
    )prg"};
    program.compile();
...
// String-complete incomplete local char array.
TEST(Compiler, incompleteLocalCharArrayFromString) {
    SourceProgram program{R"prg(
        int main() {
            char s[] = "hi";
            printf("%d %d", (int)sizeof(s), (int)s[0] + (int)s[1]);
            return 0;
        }
    )prg"};
    program.compile();
    /* "hi" + NUL = 3 bytes; 'h'+'i' = 104+105 = 209 */
    program.runAndExpect("3 209");
}

// Array designator on struct fields via [0]= style positional index? skip.
// Global incomplete array completed by brace.
