...
TEST(Compiler, driverReturnsNonZeroOnParseFailure) {
    ...
    LogManager::withOutputStreams(out, err, [&argv, &status]() {
        Driver driver {};
        status = driver.run((int)argv.size() - 1, argv.data());
    });
    EXPECT_NE(status, 0);
...
// Compile-only must fail if the object file is missing or empty.
...
// Driver compiles each source independently under -c (no link; each yields its own .o).
TEST(Compiler, multipleSourceFilesEachCompile) {
    SourceProgram a{R"prg(int printf(const char *, ...);
int scanf(const char *, ...);
        int main() {
            printf("%d", 1);
            return 0;
    ...
    SourceProgram b{R"prg(int printf(const char *, ...);
int scanf(const char *, ...);
        int main() {
            printf("%d", 2);
            return 0;
    ...
    std::vector<std::string> arguments {
            "trans", "--resources=../../../", "--no-preprocess", "-c",
    ...
    LogManager::withOutputStreams(out, err, [&argv]() {
        Driver driver {};
        driver.run((int)argv.size() - 1, argv.data());
    });
    EXPECT_THAT(err.str(), Eq(""));
    EXPECT_TRUE(fileExists(a.getSourceFilePath() + ".o"));
    EXPECT_TRUE(fileExists(b.getSourceFilePath() + ".o"));
    EXPECT_FALSE(fileExists(a.getExecutablePath()));
    EXPECT_FALSE(fileExists(b.getExecutablePath()));
}
