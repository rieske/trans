...
// Abstract parameter in prototype: int f(int);
TEST(Compiler, abstractParameterInPrototypeOnly) {
    SourceProgram program{R"prg(#include <stdio.h>
        int f(int, int *);
...
// Abstract array parameter in prototype (pop abstract array size path).
TEST(Compiler, abstractArrayParameterInPrototype) {
    SourceProgram program{R"prg(#include <stdio.h>
        int first(int a[]);
...
// type_name abstract arrays share Declarator::getFundamentalType (not a 1-dim stack).
TEST(Compiler, sizeofMultiDimTypeName) {
    SourceProgram program{R"prg(#include <stdio.h>
        int main() {
            printf("%d", (int)sizeof(int[2][3]));
            return 0;
        }
    )prg"};
    program.compile();
...
TEST(Compiler, sizeofPointerToArrayTypeName) {
    SourceProgram program{R"prg(#include <stdio.h>
        int main() {
            int row[3];
            int (*p)[3];
            p = &row;
            (*p)[1] = 7;
            printf("%d %d", (int)sizeof(int (*)[3]), (*p)[1]);
            return 0;
        }
    )prg"};
    program.compile();
...
TEST(Compiler, castThroughFunctionPointerTypeName) {
    SourceProgram program{R"prg(#include <stdio.h>
        int id(int x) { return x + 1; }
        int main() {
            void *p;
            p = (void *)id;
            printf("%d", ((int (*)(int))p)(41));
            return 0;
        }
    )prg"};
    program.compile();
...
TEST(Compiler, restrictQualifiedArrayParam) {
    SourceProgram program{R"prg(#include <stdio.h>
        int first(int n, int p[restrict n]) { return p[0]; }
        int main() {
            int v[2];
            v[0] = 42;
            v[1] = 0;
...
TEST(Compiler, abstractRestrictArrayParam) {
    SourceProgram program{R"prg(#include <stdio.h>
        int first(int [restrict]);
        int first(int p[restrict]) { return p[0]; }
        int main() {
            int v[1];
            v[0] = 6;
            printf("%d", first(v));
...
TEST(Compiler, abstractStaticArrayParam) {
    SourceProgram program{R"prg(#include <stdio.h>
        int first(int [static 1]);
        int first(int p[static 1]) { return p[0]; }
        int main() {
            int v[1];
            v[0] = 4;
            printf("%d", first(v));
...
TEST(Compiler, sizeofTypeofStar) {
    SourceProgram program{R"prg(#include <stdio.h>
...
TEST(Compiler, gnuRestrictQualifiedArrayParam) {
    SourceProgram program{R"prg(#include <stdio.h>
        int first(int n, int p[__restrict n]) { return p[0]; }
        int main() {
            int v[1];
            v[0] = 9;
            printf("%d", first(1, v));
