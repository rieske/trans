...
// Dialect-suffixed paths so parallel intel/att shards do not clobber each other.
std::string writeTmpSource(const std::string& name, const std::string& body) {
    ensureTestProgramsTmpDir();
    std::string path = getTestProgramsTmpDir() + name + "_" + functionalTestDialectTag();
...
std::string dialectTmpName(const std::string& base) {
    return getTestProgramsTmpDir() + base + "_" + functionalTestDialectTag();
}
...
int runDriver(const std::vector<std::string>& arguments, std::string* errOut = nullptr) {
    // Inject assembly dialect after "trans" so multi-TU shards honor TRANS_ASM_DIALECT.
    std::vector<std::string> args = arguments;
    if (!args.empty()) {
        args.insert(args.begin() + 1, "-a" + functionalTestDialectTag());
    }
...
TEST(MultiTu, defineInOneTuCallFromAnother) {
    ...
    std::string exe = dialectTmpName("multi_tu_linked.out");
...
TEST(MultiTu, staticDoesNotExportAcrossTu) {
    ...
    std::string libObj = libSrc + ".o";
    std::string mainObj = mainSrc + ".o";
    std::string exe = dialectTmpName("multi_tu_static.out");
    remove(libObj.c_str());
    remove(mainObj.c_str());
    remove(exe.c_str());
...
TEST(MultiTu, externDataDefinedInOneTu) {
    ...
    std::string mainObj = mainSrc + ".o";
    std::string exe = dialectTmpName("multi_tu_data.out");
    std::string outputFile = exe + ".execution.output";
...
TEST(MultiTu, duplicateGlobalDefinitionFailsToLink) {
    ...
    std::string exe = dialectTmpName("multi_tu_dup.out");
    remove(aObj.c_str());
    remove(bObj.c_str());
    remove(exe.c_str());
