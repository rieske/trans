...
// &a[s.x] must still load the index member; only the addressed element is not loaded.
TEST(Compiler, addressOfSubscriptWithMemberIndex) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct S { int x; };
        int main() {
            int a[8];
            ...
            s.x = 3;
            p = &a[s.x];
            printf("%d %d", (int)(p - a), *p);
            return 0;
        }
    )prg"};
    program.compile();
    program.runAndExpect("3 11");
}

// Short/char stores through pointer (lvalueAssign accessSize 1/2).
TEST(Compiler, shortAndCharPointerStores) {
    SourceProgram program{R"prg(#include <stdio.h>
        int main() {
            short s;
            unsigned short us;
            ...
            *cp = 5;
            *ucp = 7;
            printf("%d %d %d %d", (int)s, (int)us, (int)c, (int)uc);
            return 0;
        }
    )prg"};
    program.compile();
    program.runAndExpect("10 20 5 7");
}
