...
// Storage-class-only then type via separate — bare static at block is invalid;
// file-scope static int hits storage class builder paths.
TEST(Compiler, fileScopeStaticOnlyStorageThenInit) {
    SourceProgram program{R"prg(
        static int g = 42;
        int main() {
            printf("%d", g);
