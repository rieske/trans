...
// Multi-word store via pointer (struct assign through *p).
TEST(Compiler, multiWordStructStoreThroughPointer) {
    SourceProgram program{R"prg(#include <stdio.h>
        ...
        int main() {
            ...
            struct Big dst;
            struct Big *p = &dst;
            src.a = 10; src.b = 20; src.c = 12;
            *p = src;
            printf("%ld", dst.a + dst.b + dst.c);
            return 0;
        }
    )prg"};
    program.compile();
