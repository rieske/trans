...
// Struct and scalar are not assignment-compatible (productAssignFrom rejects).
...
// lvalue-required errors (prefix/postfix on non-lvalue).
TEST(SemanticErrors, incrementNonLvalueIsError) {
    SourceProgram program{R"prg(
        int main() {
            int x;
            x = 1;
...
TEST(SemanticErrors, postfixIncrementNonLvalueIsError) {
    SourceProgram program{R"prg(
        int main() {
            int x;
            x = 1;
...
// Invalid unary * on non-pointer.
TEST(SemanticErrors, derefNonPointerIsError) {
    SourceProgram program{R"prg(
        int main() {
            int x;
            x = 1;
    ...
    program.compile();
    program.assertCompilationErrors("unary *");
}

// Invalid [] on non-array/pointer.
TEST(SemanticErrors, invalidSubscriptIsError) {
    SourceProgram program{R"prg(
        int main() {
            int x;
            x = 1;
...
// Multiple storage classes is a semantic error.
TEST(SemanticErrors, multipleStorageClassesIsError) {
    SourceProgram program{R"prg(
        int main() {
            static auto int x;
            x = 1;
    ...
    program.compile();
    program.assertCompilationErrors("multiple storage classes");
}

// Global variable conflicts with function.
TEST(SemanticErrors, globalVariableConflictsWithFunction) {
    SourceProgram program{R"prg(
        int f(void);
        int f;
        int main() { return 0; }
    ...
    program.compile();
    program.assertCompilationErrors("conflicts with function");
}

// sizeof on incomplete/failed operand recovery path: sizeof undeclared.
TEST(SemanticErrors, sizeofUndeclaredIsError) {
    SourceProgram program{R"prg(
        int main() {
...
// String literal initializer on wrong type.
TEST(SemanticErrors, stringLiteralOnIntIsError) {
    SourceProgram program{R"prg(
        int g = "hi";
        int main() { return 0; }
    )prg"};
    program.compile();
    program.assertCompilationErrors("string literal initializer");
}

// Prototype (not definition) conflicts with global variable — DeclarationAnalyzer path.
TEST(SemanticErrors, prototypeConflictsWithGlobalVariable) {
    SourceProgram program{R"prg(
        int f;
        int f(void);
        int main() { return 0; }
...
TEST(SemanticErrors, incompatibleFunctionRedeclaration) {
    SourceProgram program{R"prg(
        int f(int);
        int f(double);
        int main() { return 0; }
