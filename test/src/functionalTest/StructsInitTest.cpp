...
// git ref-filter: struct object_info empty = OBJECT_INFO_INIT ({ 0 }).
// Enum members leave 4-byte padding before a following union; field-wise zero
// stores miss that gap. memcmp(&empty, &bss_zero, sizeof) must succeed after
// stack poison (for-each-ref missing-object on broken refs).
TEST(Compiler, structBraceZeroInitZerosInterMemberPadding) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct ObjectInfo {
            void *typep;
            void *sizep;
            ...
            void *contentp;
            void *mtimep;
            enum { OI_CACHED, OI_LOOSE, OI_PACKED } whence;
            union {
                struct {
                    void *pack;
                    long offset;
                    enum { PACKED_UNKNOWN, PACKED_FULL } type;
                } packed;
            } u;
        };
        void poison_stack(void) {
        ...
        int main() {
            static struct ObjectInfo zero;
            int i;
            int nonzero;
            unsigned char *p;
            poison_stack();
            {
                struct ObjectInfo empty = { 0 };
                p = (unsigned char *)&empty;
                nonzero = 0;
                for (i = 0; i < (int)sizeof(empty); i = i + 1) {
                    if (p[i] != 0) {
                        nonzero = nonzero + 1;
                    }
                }
...
TEST(Compiler, localStructBraceInitPointerToStructMember) {
    ...
    SourceProgram program2{R"prg(#include <stdio.h>
        struct Big {
            long a;
            long b;
            long c;
...
// Extra braces around the member string (C 6.7.9), not a peel into s[0].
TEST(Compiler, localStructCharArrayNestedBracedString) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct T {
            char s[8];
            int n;
        ...
        int main() {
            struct T t = { { "ab" }, 3 };
            printf("%s %d %d", t.s, t.n, (int)t.s[0]);
            return 0;
        }
    )prg", "local_struct_nested_brace_str"};
    program.compile();
    program.runAndExpect("ab 3 97");
}
...
// Designator names a wrapping record whose first member is char[N]: string must
// pack into the nested array (brace elision), not peel to a single char.
TEST(Compiler, designatedNestedStructStringElidesToCharArray) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct Inner {
            char c[8];
            int flag;
        ...
        int main() {
            struct Outer o = { .tag = 1, .in = "xy" };
            printf("%d %c %c %d %d",
                o.tag, o.in.c[0], o.in.c[1], (int)o.in.c[2], o.in.flag);
            return 0;
        }
    )prg"};
    program.compile();
    program.runAndExpect("1 x y 0 0");
}

// git parse-options: .defval = (intptr_t)"all" stores the string address as an
// integer word in a static option table (not a char[] pack).
TEST(Compiler, globalStructDefvalStringAddressAsInteger) {
    SourceProgram program{R"prg(#include <stdio.h>
        ...
        int main() {
            const char *p = (const char *)opts[0].defval;
            printf("%d %d %s", opts[0].type, opts[0].value == (void *)&mode, p);
            return 0;
        }
    )prg"};
    program.compile();
    program.runAndExpect("1 1 all");
}

// Scalar form of the same cast (file-scope intptr_t holding a string address).
TEST(Compiler, globalIntptrCastOfStringLiteral) {
    SourceProgram program{R"prg(#include <stdio.h>
        typedef long intptr_t;
        static intptr_t def = (intptr_t)"hi";
        int main() {
            printf("%s", (const char *)def);
            return 0;
        }
    )prg"};
    program.compile();
...
// Scalar brace initializer (C: int x = { 7 };).
...
// File-scope scalar brace.
TEST(Compiler, globalScalarBraceInitializer) {
    SourceProgram program{R"prg(#include <stdio.h>
        int g = { 42 };
        int main() {
            printf("%d", g);
...
// Global scalar brace with excess elements.
TEST(Compiler, globalScalarBraceExcessIsError) {
    SourceProgram program{R"prg(
        int g = { 1, 2 };
        int main() { return 0; }
    )prg"};
...
// Nested designated member path on local struct.
TEST(Compiler, nestedDesignatedMemberInit) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct Inner { int x; int y; };
        struct Outer { struct Inner in; int z; };
        int main() {
            struct Outer o = { .in.x = 10, .in.y = 20, .z = 12 };
            printf("%d", o.in.x + o.in.y + o.z);
            return 0;
        }
    )prg"};
    program.compile();
...
// Partial struct brace with nested incomplete zeroing (large tail).
TEST(Compiler, largeStructPartialBraceZeroFill) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct Big {
            int a;
            long pad1;
        ...
        int main() {
            struct Big s = { 42 };
            printf("%d %ld %d", s.a, s.pad2, s.b);
            return 0;
        }
    )prg"};
    program.compile();
    program.runAndExpect("42 0 0");
}

// Array-of-struct with nested brace elements.
TEST(Compiler, nestedBraceArrayOfStructInit) {
    SourceProgram program{R"prg(#include <stdio.h>
        struct Cell { int v; int w; };
        int main() {
            struct Cell a[2] = { { 10, 11 }, { 12, 9 } };
            printf("%d", a[0].v + a[0].w + a[1].v + a[1].w);
            return 0;
        }
    )prg"};
    program.compile();
