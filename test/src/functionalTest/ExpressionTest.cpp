...
// Large unsigned constant immediates (hex path for values > signed 32-bit).
TEST(Compiler, largeUnsignedConstantImmediate) {
    SourceProgram program{R"prg(
        int main() {
            unsigned long v = 0x80000000UL;
            printf("%d", (int)(v >> 31));
            return 0;
        }
    )prg"};
...
// Signed vs unsigned comparison (hits below-or-equal unsigned jump).
TEST(Compiler, unsignedComparisonBranch) {
    SourceProgram program{R"prg(
        int main() {
            unsigned int a = 3;
            unsigned int b = 5;
