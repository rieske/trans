...
TEST(Preprocessor, linuxInt128TypedefsParseAndUse) {
    ...
    program.compile();
    program.runAndExpect("1 2 16 16");
}
...
// arrays. Lower to a real stack allocation (not malloc).
...
// __func__ / __FUNCTION__ / __PRETTY_FUNCTION__ lower to the current function name.
TEST(Preprocessor, mapsFuncIdentifiersToCurrentFunctionName) {
    SourceProgram program{R"prg(#include <stdio.h>
        int main() {
...
// Hit CG bswap, width-aware ctz, and real alloca.
TEST(Compiler, builtinBswapCtzAndAlloca) {
    SourceProgram program{R"prg(#include <stdio.h>
        int main() {
            unsigned int u = __builtin_bswap32(0x12345678u);
            unsigned short s = __builtin_bswap16(0x1234);
            unsigned long l = __builtin_bswap64(0x0102030405060708ul);
            int z = __builtin_ctz(8);
            int zl = __builtin_ctzl(16ul);
            int zll = __builtin_ctzll(32ull);
            void *p = __builtin_alloca(32);
            *(char *)p = 7;
            printf("%u %u %lu %d %d %d %d", u, (unsigned)s, l, z, zl, zll, (int)*(char *)p);
            return 0;
        }
    )prg"};
    program.compile();
    program.runAndExpect("2018915346 13330 578437695752307201 3 4 5 7");
}
