        ...
        while (i_ptr != NULL)
        {
            ...
            if (expected->size() && is_nonterminal(expected->at(0)))     // [ A -> u.Bv, a ] (expected[0] == B)
            {
                ...
                if ( (expected->size() > 1) && is_nonterminal(expected->at(1)) )     // v - neterminalas
                {
                    ...
                    for(vector<string *>::const_iterator it = first_table->at(expected->at(1))->begin(); it != first_table->at(expected->at(1))->end(); it++)
                ...
                {
                    for (vector<string *>::const_iterator it = i_ptr->getItem()->getLookaheads()->begin(); it != i_ptr->getItem()->getLookaheads()->end(); it++)
                        first_va_.push_back(*it);
                }
                ...
                while (g_ptr != NULL)                   // einam per gramatikos taisykles
                {
                    ...
                    {
                        for (vector<string *>::const_iterator lookahead_iterator = first_va_.begin(); lookahead_iterator != first_va_.end(); lookahead_iterator++)
                        {
