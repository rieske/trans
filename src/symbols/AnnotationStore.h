...
//
// Packaging:
//   AnnotationTypes.h — ValueSlot/LabelSlot/StringSlot, FieldInit, FunctionFrame
//   AddressPlan / CallPlan / BuiltinPlan / PointerArithPlan — SA→CG plans
//   This store — map ownership + accessors
//
// All decorations here are SA→CG product (codegen reads them). va_arg type_name
// lives on FunctionCall; offsetof is a parse-time ConstantExpression.
...
struct NodeAnnotations {
    ...
    std::vector<FieldInit> fieldInitPlan;
...
public:
    ...
    void addFieldInit(NodeRef node, FieldInit init);
    const std::vector<FieldInit>& fieldInits(NodeRef node) const;
...
private:
    ...
    static const std::vector<FieldInit> kEmptyFieldInits;
};
