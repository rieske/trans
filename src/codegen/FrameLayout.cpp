...
std::vector<Value> packFrameValues(
    ...
    for (const auto& fs : symbols) {
        ...
        const int align = std::max(fs.type.getAlignment(), 1);
        const int slot = type::object_abi::takeAlignedWords(nextSlot, align, words);
        values.push_back(fs.toValueAtSlot(slot));
    }
