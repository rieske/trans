...
void emitRealCall(CodeGeneratingVisitor& self, ast::FunctionCall& functionCall,
    ...
    // Operand may be an undeclared builtin designator with no SA result.
    if (functionCall.getOperandExpression()->hasResultSymbol(store)) {
    ...
    if (!fn->returnType().isVoid()) {
        if (type::object_abi::typeNeedsMemoryReturn(fn->returnType())) {
            memoryReturnDest = functionCall.getResultSymbol(store)->getName();
        }
    }
    ...
    if (indirect) {
        calleeName = functionCall.getOperandExpression()->getResultSymbol(store)->getName();
    } else {
    ...
    if (!fn->returnType().isVoid()) {
        self.emit(ir::retrieve(
                functionCall.getResultSymbol(store)->getName(), !memoryReturnDest.empty()));
    }
}
...
void emitBuiltinPlan(CodeGeneratingVisitor& self, ast::FunctionCall& functionCall,
    ...
    const std::string result = functionCall.getResultSymbol(store)->getName();
    ...
    std::visit(
            [&](const auto& arm) {
                ...
                if constexpr (std::is_same_v<T, symbols::VaStartPlan>) {
                    ...
                    self.emit(ir::vaStart(args[0]->getResultSymbol(store)->getName()));
                    ...
                    self.emit(ir::vaCopy(args[0]->getResultSymbol(store)->getName(),
                            args[1]->getResultSymbol(store)->getName()));
                } else if constexpr (std::is_same_v<T, symbols::VaArgPlan>) {
                    if (args.empty()) {
                        requireArgCount("VaArg", args.size(), 1);
                    }
                    self.generateExpression(*args[0]);
                    self.emit(ir::vaArg(args[0]->getResultSymbol(store)->getName(), result));
                } else if constexpr (std::is_same_v<T, symbols::AllocaPlan>) {
                    if (args.empty()) {
                        requireArgCount("Alloca", args.size(), 1);
                    }
                    self.emit(ir::allocaBytes(self.generateExpression(*args[0]), result));
                } else if constexpr (std::is_same_v<T, symbols::CtzPlan>) {
                    if (args.empty()) {
                        requireArgCount("Ctz", args.size(), 1);
                    }
                    self.emit(ir::ctz(self.generateExpression(*args[0]), result, arm.widthBytes));
...
void CodeGeneratingVisitor::visit(ast::FunctionCall& functionCall) {
    ...
    if (!planPtr) {
        if (!functionCall.hasResultSymbol(store_)) {
            return;
        }
