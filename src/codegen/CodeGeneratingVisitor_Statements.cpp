...
void CodeGeneratingVisitor::visit(ast::LoopStatement& loop) {
    if (loop.header->bodyBeforeTest()) {
        ...
        if (auto* test = loop.header->testExpression()) {
            ...
            instructions.push_back(std::make_unique<ZeroCompare>(test->getResultSymbol()->getName()));
            instructions.push_back(std::make_unique<Jump>(
                    loop.header->getLoopEntry()->getName(), JumpCondition::IF_NOT_EQUAL));
        }
