...
void StackMachine::startProcedure(const Procedure& procedure) {
    ...
    for (auto& argument : arguments) {
        ...
        const int home = type::object_abi::takeAlignedWords(
                localIndex, argument.getClassification().alignBytes, words);
        Value registerArgument { argument.getName(), home, argument.getValueKind(),
    ...
    const auto layout = type::object_abi::frameLayout(maxWordEnd, savedRegistersStack);
    localVariableStackSize = layout.homeBytes;
    frameSubAmount_ = layout.subBytes;
    assembly << instructionSet->sub(registers->getStackPointer(), frameSubAmount_, GprWidth::W64);
