...
void StackMachine::startProcedure(std::string procedureName, std::vector<Value> values, std::vector<Value> arguments,
    ...
    if (variadic) {
        ...
        for (std::size_t i = 0; i < arguments.size(); ++i) {
            ...
            if (words == 1 && i < regArgCapacity) {
                ...
                Value stackArgument { argument.getName(), stackIndex, argument.getValueKind(),
        ...
        for (auto& argument : arguments) {
            const int words = type::object_abi::valueWords(argument.getSizeInBytes());
            if (words == 1 && argument.getValueKind() == ValueKind::FLOATING && floatingRegArgNames.size() < 8) {
    ...
    if (variadic) {
        for (std::size_t i = 0; i < regArgCapacity; ++i) {
            std::string homeName;
            if (i < arguments.size()) {
                const int words = type::object_abi::valueWords(arguments[i].getSizeInBytes());
                if (words == 1) {
