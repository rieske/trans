...
void StackMachine::wideIntegerBinary(Value& left, Value& right, Value& result, WideIntegerOp op) {
    ...
    for (int w = 0; w < words; ++w) {
        ...
        case WideIntegerOp::Add:
            assembly << (carry ? instructionSet->adc(tmp, acc) : instructionSet->add(tmp, acc, GprWidth::W64));
            break;
        case WideIntegerOp::Sub:
            assembly << (carry ? instructionSet->sbb(tmp, acc) : instructionSet->sub(tmp, acc, GprWidth::W64));
            break;
        case WideIntegerOp::And:
            assembly << instructionSet->and_(tmp, acc, GprWidth::W64);
            break;
        case WideIntegerOp::Or:
            assembly << instructionSet->or_(tmp, acc, GprWidth::W64);
            break;
        case WideIntegerOp::Xor:
            assembly << instructionSet->xor_(tmp, acc, GprWidth::W64);
            break;
        }
...
void StackMachine::wideUnaryMinus(Value& operand, Value& result) {
    ...
    assembly << instructionSet->xor_(zero, zero, GprWidth::W64);
    assembly << instructionSet->neg(lo, GprWidth::W64);
    assembly << instructionSet->adc(zero, hi);
    assembly << instructionSet->neg(hi, GprWidth::W64);
...
void StackMachine::wideUnaryNot(Value& operand, Value& result) {
    ...
    for (int w = 0; w < words; ++w) {
        ...
        assembly << instructionSet->not_(acc, GprWidth::W64);
        storeWord(acc, result, w);
    }
}
...
void StackMachine::setCompareFlagsFromTernary(Register& acc, bool signedRel) {
    if (signedRel) {
        assembly << instructionSet->cmp(acc, 0, GprWidth::W64);
    } else {
        assembly << instructionSet->add(acc, 1, GprWidth::W64);
        assembly << instructionSet->cmp(acc, 1, GprWidth::W64);
    }
}
...
void StackMachine::wideShift(Value& value, Value& count, Value& result, WideShiftOp op) {
    ...
    assembly << instructionSet->cmp(rcx, 63, GprWidth::W64);
    assembly << instructionSet->ja(ge64);
    switch (op) {
    case WideShiftOp::Left:
        assembly << instructionSet->shld(lo, hi);
        assembly << instructionSet->shl(lo, GprWidth::W64);
        break;
    case WideShiftOp::ArithmeticRight:
        assembly << instructionSet->shrd(hi, lo);
        assembly << instructionSet->sar(hi, GprWidth::W64);
        break;
    case WideShiftOp::LogicalRight:
        assembly << instructionSet->shrd(hi, lo);
        assembly << instructionSet->shr(hi, GprWidth::W64);
        break;
    }
    assembly << instructionSet->jmp(done);
    assembly.label(instructionSet->label(ge64));
    assembly << instructionSet->sub(rcx, 64, GprWidth::W64);
    switch (op) {
    case WideShiftOp::Left:
        assembly << instructionSet->mov(lo, hi);
        assembly << instructionSet->xor_(lo, lo, GprWidth::W64);
        assembly << instructionSet->shl(hi, GprWidth::W64);
        break;
    case WideShiftOp::ArithmeticRight:
        assembly << instructionSet->mov(hi, lo);
        assembly << instructionSet->sar(lo, GprWidth::W64);
        assembly << instructionSet->mov("63", rcx);
        assembly << instructionSet->sar(hi, GprWidth::W64);
        break;
    case WideShiftOp::LogicalRight:
        assembly << instructionSet->mov(hi, lo);
        assembly << instructionSet->shr(lo, GprWidth::W64);
