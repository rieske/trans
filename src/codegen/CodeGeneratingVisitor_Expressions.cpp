...
void CodeGeneratingVisitor::emitIncDec(const std::string& name, const type::Type& valueType,
    ...
    const int step = pointerIncrementAmount(valueType);
    if (increment) {
...
void CodeGeneratingVisitor::visit(ast::PostfixExpression& expression) {
    ...
    if (expression.getOperandExpression()->lvalueAnnotation(store_)) {
...
void CodeGeneratingVisitor::visit(ast::OffsetofExpression& expression) {
    long value = 0;
    if (!expression.evaluateConstant(value) || !expression.hasResult(store_)) {
        throw std::runtime_error { "OffsetofExpression missing SA fold" };
    }
    emit(ir::assignConstant(std::to_string(value), expression.result(store_)->getName()));
}
...
void CodeGeneratingVisitor::visit(ast::UnaryExpression& expression) {
    ...
    case OperatorKind::Deref: {
        ...
        // *fp: result aliases the pointer ValueEntry (no load).
        ...
        emit(ir::dereference(addrSrc, ptrName, expression.result(store_)->getName()));
        break;
    }
...
void CodeGeneratingVisitor::visit(ast::TypeCast& expression) {
    ...
    emitConvert(expression.getOperandExpression()->result(store_)->getName(),
            expression.result(store_)->getName(), source, target);
...
void CodeGeneratingVisitor::visit(ast::ArithmeticExpression& expression) {
    ...
    if (op == OperatorKind::Add || op == OperatorKind::Sub) {
        ...
        emitMulDiv(op, left, right, result,
                expression.result(store_)->getType(), unsignedDiv);
    } else {
...
void CodeGeneratingVisitor::visit(ast::ShiftExpression& expression) {
    ...
    emitBinaryOp(*this, op, left, right, expression.result(store_)->getName(), false, logical);
    narrowIntegralResult(expression.result(store_)->getType(), expression.result(store_)->getName());
}
...
void CodeGeneratingVisitor::visit(ast::BitwiseExpression& expression) {
    ...
    emitBinaryOp(*this, op, left, right, expression.result(store_)->getName());
    narrowIntegralResult(expression.result(store_)->getType(), expression.result(store_)->getName());
}
...
void CodeGeneratingVisitor::visit(ast::MemberAccess& expression) {
    ...
    emitFieldFromPlan(*field, *expression.getBase(),
            expression.lvalueAnnotation(store_)->getName(), *this, store_);
}
...
void CodeGeneratingVisitor::visit(ast::ArrayAccess& arrayAccess) {
    ...
    if (!arrayAccess.hasResult(store_)) {
        return;
    }
    ...
    if (!arrayAccess.lvalueAnnotation(store_)) {
        throw std::runtime_error { "ArrayAccess missing lvalue from SA" };
    }
    emitIndexFromPlan(*idx, *arrayAccess.getLeftOperand(),
            arrayAccess.getRightOperand()->result(store_)->getName(),
            arrayAccess.lvalueAnnotation(store_)->getName(), *this, store_);
}
...
void CodeGeneratingVisitor::visit(ast::InitializerListExpression& expression) {
    ...
    for (const auto& element : expression.getElements()) {
        if (element.value && element.value->hasResult(store_)) {
...
void CodeGeneratingVisitor::visit(ast::IdentifierExpression& identifier) {
    if (const std::string* label = store_.string(&identifier, symbols::StringSlot::ConstantLabel)) {
        emit(ir::assignLabelAddress(
                *label, identifier.result(store_)->getName()));
        return;
    }
...
void CodeGeneratingVisitor::visit(ast::ConstantExpression& constant) {
    ...
    const std::string resultName = constant.result(store_)->getName();
