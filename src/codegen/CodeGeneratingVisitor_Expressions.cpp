...
void CodeGeneratingVisitor::visit(ast::UnaryExpression& expression) {
    ...
    case OperatorKind::Deref: {
        ...
        if (sameValueTemp(expression.getResultSymbol(store_), operand->getResultSymbol(store_))) {
            break;
        }
        // Materialize a pointer to the pointee, then one load path.
        const std::string ptrName = expression.getLvalueSymbol(store_)->getName();
        std::string addrSrc = operand->getResultSymbol(store_)->getName();
        ...
        emit(ir::dereference(addrSrc, ptrName, expression.getResultSymbol(store_)->getName()));
        break;
    }
