...
void CodeGeneratingVisitor::visit(ast::PostfixExpression& expression) {
    ...
    const int amount = pointerIncrementAmount(expression.operandType());
    ...
    if (auto* lvalue = expression.operandLvalueSymbol()) {
        instructions.push_back(std::make_unique<LvalueAssign>(
                ...
                type::memoryAccessSizeBytes(expression.operandType())));
    }
...
void CodeGeneratingVisitor::visit(ast::ArithmeticExpression& expression) {
    ...
    const bool unsignedDiv =
            type::isUnsignedSide(expression.leftOperandType())
            || type::isUnsignedSide(expression.rightOperandType());
    const OperatorKind op = expression.getOperator()->getKind();
...
void CodeGeneratingVisitor::visit(ast::ShiftExpression& expression) {
    ...
    const type::Type& shiftedType = expression.getResultSymbol()->getType();
    const bool logical = shiftedType.isPrimitive() && !shiftedType.getPrimitive().isSigned();
...
void CodeGeneratingVisitor::visit(ast::ComparisonExpression& expression) {
    ...
    const bool unsignedCompare =
            type::isUnsignedSide(expression.leftOperandType()) || type::isUnsignedSide(expression.rightOperandType());
    ...
    if (unsignedCompare) {
        const int leftW = typeWidth(expression.leftOperandType());
        const int rightW = typeWidth(expression.rightOperandType());
...
void CodeGeneratingVisitor::visit(ast::AssignmentExpression& expression) {
    ...
    if (assignKind == OperatorKind::Assign) {
        if (expression.leftOperandLvalueSymbol()) {
            ...
            instructions.push_back(std::make_unique<LvalueAssign>(
                        ...
                        type::memoryAccessSizeBytes(expression.leftOperandType())
            ));
        } else {
            instructions.push_back(std::make_unique<Assign>(
                        expression.rightOperandSymbol()->getName(),
    ...
    const bool unsignedDiv =
            type::isUnsignedSide(expression.leftOperandType())
            || type::isUnsignedSide(expression.rightOperandType());
    const type::Type& lhsType = expression.leftOperandType();
    const bool logicalShr = lhsType.isPrimitive() && !lhsType.getPrimitive().isSigned();
    ...
    if (auto* lvalue = expression.leftOperandLvalueSymbol()) {
        instructions.push_back(std::make_unique<LvalueAssign>(
                resultName, lvalue->getName(), type::memoryAccessSizeBytes(expression.leftOperandType())));
    }
}
...
void CodeGeneratingVisitor::visit(ast::MemberAccess& expression) {
    ...
    if (!baseIsPointer) {
        if (auto* baseLval = expression.getBase()->getLvalueSymbol()) {
            baseName = baseLval->getName();
            baseIsPointer = true;
        }
    }
    ...
    // Array members are not loaded: the field address is the array object.
    // Still copy that address into the result temporary so rvalue uses
    // (casts, assignments) see a defined pointer value rather than garbage
    ...
    if (expression.getType().isArray()) {
        ...
        instructions.push_back(std::make_unique<Dereference>(
                ...
                type::memoryAccessSizeBytes(expression.getType()),
                type::memoryAccessIsSigned(expression.getType())));
    }
}
