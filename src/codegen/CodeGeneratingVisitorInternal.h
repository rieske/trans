...
// Resolve member base for FieldAddress: dot may already be an address lvalue.
inline void resolveMemberBase(std::string& baseName, bool& baseIsPointer, ast::Expression* baseExpr) {
    if (!baseIsPointer && baseExpr) {
        if (auto* baseLval = baseExpr->getLvalueSymbol()) {
...
using code_gen_detail::resolveMemberBase;
