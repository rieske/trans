...
void StackMachine::copyBytes(Register& srcBase, Register& destBase, int n,
    ...
    while (off < n) {
        ...
        const int chunk = remain >= MACHINE_WORD_SIZE ? MACHINE_WORD_SIZE
                : remain >= 4 ? 4
                : remain >= 2 ? 2 : 1;
        assembly << instructionSet->load(MemoryOperand::at(srcBase, off), tmp, chunk, false);
        assembly << instructionSet->store(tmp, MemoryOperand::at(destBase, off), chunk);
        off += chunk;
    }
}
...
int StackMachine::emitCallArguments(std::size_t firstReg) {
    ...
    if (argumentOffset) {
        assembly << instructionSet->sub(registers->getStackPointer(), argumentOffset, GprWidth::W64);
    }
    ...
    if (counts.sseRegs == 0) {
        assembly << instructionSet->xor_(rax, rax, GprWidth::W64);
    } else {
...
void StackMachine::emitCall(const CallTarget& target, const std::string& memoryReturnDest) {
    ...
    if (argumentOffset) {
        assembly << instructionSet->add(registers->getStackPointer(), argumentOffset, GprWidth::W64);
    }
}
