...
const FunctionDeclarator* FunctionDeclarator::innermostFunctionDeclarator() const {
    if (const FunctionDeclarator* inner = baseDeclarator->innermostFunctionDeclarator()) {
        return inner;
    }
    return this;
}
...
type::Type FunctionDeclarator::getFundamentalType(std::vector<Pointer> indirection, const type::Type& baseType) const {
    ...
    for (const auto& argument : formalArguments) {
        if (auto argumentType = argument.tryGetType()) {
            argumentTypes.push_back(std::move(*argumentType));
        } else {
            argumentTypes.push_back(type::signedInteger());
        }
    }
...
void FunctionDeclarator::foldArrayBoundSizeofs(const std::function<void(Expression*)>& foldSizeof) {
    baseDeclarator->foldArrayBoundSizeofs(foldSizeof);
}

bool FunctionDeclarator::hasArrayDeclarator() const {
    return baseDeclarator->hasArrayDeclarator();
}
