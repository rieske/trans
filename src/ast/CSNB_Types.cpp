...
void typeSpecTypeof(AbstractSyntaxTreeBuilderContext& context) {
    ...
    // Operand is owned by TypeSpecifier; SA resolves the real type (not a fake ulong).
    context.popTerminal(); // )
    context.popTerminal(); // (
    auto operand = context.popExpression();
    auto typeofKw = context.popTerminal(); // __typeof__
    (void)typeofKw;
    context.pushTypeSpecifier(TypeSpecifier::makeTypeof(std::shared_ptr<Expression>(std::move(operand))));
}
