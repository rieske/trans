...
void builtinOffsetof(AbstractSyntaxTreeBuilderContext& context) {
    ...
    if (typeSpec.isTypeof() && typeSpec.getTypeofOperand()) {
        Expression* raw = typeSpec.getTypeofOperand();
...
void builtinVaArg(AbstractSyntaxTreeBuilderContext& context) {
    ...
    auto call = std::make_unique<FunctionCall>(
            ...
            std::move(args));
    // Type is not an expression argument; store on the call node for SA.
    // Concrete type_names only: typeof is provisional void until SA (not used for va_arg in product).
