...
void sizeofTypeExpression(AbstractSyntaxTreeBuilderContext& context) {
    ...
    // sizeof(typeof(...)) is not folded at parse: typeof is provisional until SA.
    // Product does not yet re-fold sizeof-type in SA; emit 0 for typeof type_names.
    int size = typeSpec.isTypeof() ? 0 : typeSpec.getType().getSize();
    if (size < 0) {
        size = 0;
    }
...
void genericAssociationType(AbstractSyntaxTreeBuilderContext& context) {
    ...
    context.popTerminal(); // :
    auto typeSpec = context.popTypeSpecifier();
    (void)typeSpec; // association type not yet applied in the product
