...
// C 6.4.4.1: first type in the suffix/base list that can represent the value.
...
void identifierExpression(AbstractSyntaxTreeBuilderContext& context) {
    ...
    if (!context.environment().lookupObject(identifier.value)
            && context.environment().lookupEnumConstant(identifier.value, ice)) {
...
void functionCall(AbstractSyntaxTreeBuilderContext& context) {
    context.popTerminal(); // )
    context.popTerminal(); // (
    auto arguments = context.popActualArgumentsList();
    auto callee = context.popExpression();
    context.pushExpression(std::make_unique<FunctionCall>(std::move(callee), std::move(arguments)));
}
...
void sizeofTypeExpression(AbstractSyntaxTreeBuilderContext& context) {
    ...
    auto resolved = typeName.tryResolve(context.environment());
    auto sizeofExpr = std::make_unique<UnaryExpression>(
            Operator::makeUnary("sizeof"),
            std::make_unique<TypeNameExpression>(std::move(typeName), sizeofKw.context));
    if (resolved) {
        if (auto bytes = type::sizeofObject(*resolved, context.environment().gnuExtensions())) {
            sizeofExpr->setFoldedInteger(*bytes);
        }
    }
    context.pushExpression(std::move(sizeofExpr));
}
