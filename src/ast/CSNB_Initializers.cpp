...
void compoundLiteralExpression(AbstractSyntaxTreeBuilderContext& context) {
    // Grammar: ( type_name ) { initializer_list } — initializer_list always reduces to
    // InitializerListExpression (via singleElement / addTo list creators).
    context.popTerminal(); // }
    ...
    if (!list) {
        throw std::runtime_error { "compound literal expected initializer list" };
    }
    initExpr.release();
...
void designatorListAppend(AbstractSyntaxTreeBuilderContext& context) {
    ...
    base.insert(base.end(), std::make_move_iterator(suffix.begin()),
...
void initializedDeclaration(AbstractSyntaxTreeBuilderContext& context) {
    ...
    // PE owns lexer-feedback registration (typedef aliases and object-name shadows).
...
void declaration(AbstractSyntaxTreeBuilderContext& context) {
    context.popTerminal();
    auto declarationSpecifiers = context.popDeclarationSpecifiers();
    context.environment().session().transparentUnion.discard();
