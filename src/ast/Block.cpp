...
Block::Block(std::vector<std::unique_ptr<AbstractSyntaxTreeNode>> items) : items{std::move(items)} {}
...
void Block::accept(AbstractSyntaxTreeVisitor &visitor) { visitor.visit(*this); }
...
const std::vector<std::unique_ptr<AbstractSyntaxTreeNode>> &Block::getItems() const { return items; }
