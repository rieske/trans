...
void forLoop(AbstractSyntaxTreeBuilderContext& context, ForInitKind init,
    ...
    // <decl> already includes the terminating ';'.
...
template<ForInitKind Init, bool HasClause, bool HasIncrement, typename Register, typename Rhs>
void bindForLoop(Register&& registerFor, Rhs&& forRhs) {
    registerFor(forRhs(Init, HasClause, HasIncrement),
            [](AbstractSyntaxTreeBuilderContext& context) {
                forLoop(context, Init, HasClause, HasIncrement);
            });
}
...
ContextualSyntaxNodeBuilder::ContextualSyntaxNodeBuilder(const parser::Grammar& grammar) {
    ...
    bind(s_direct_declarator, { s_direct_declarator, s_open_bracket, s_conditional_exp, s_close_bracket }, arrayDeclarator);
    ...
    bind(s_direct_declarator, { s_direct_declarator, s_open_bracket, s_type_qualifier_list, s_conditional_exp, s_close_bracket }, arrayDeclaratorQualified);
    ...
    bind(s_direct_abstract_declarator, { s_direct_abstract_declarator, s_open_bracket, s_conditional_exp, s_close_bracket }, arrayDeclarator);
    bind(s_direct_abstract_declarator, { s_open_bracket, s_conditional_exp, s_close_bracket }, abstractArrayOnlySized);
    ...
    bind(s_direct_abstract_declarator, { s_direct_abstract_declarator, s_open_bracket, s_type_qualifier_list, s_conditional_exp, s_close_bracket }, arrayDeclaratorQualified);
    bind(s_direct_abstract_declarator, { s_open_bracket, s_type_qualifier_list, s_conditional_exp, s_close_bracket }, abstractArrayOnlyQualifiedSized);
    ...
    bind(s_id_list, { s_identifier }, knrIdentifierParameterLists);
    bind(s_id_list, { s_id_list, s_comma, s_identifier }, knrIdentifierParameterLists);
    bind(s_direct_declarator, { s_direct_declarator, s_open_paren, s_id_list, s_close_paren }, knrIdentifierParameterLists);
    ...
    bind(s_function_definition, { s_decl_specs, s_declarator, s_decl_list, s_compound_stat }, knrStyleFunctionDefinitions);
    bind(s_function_definition, { s_declarator, s_decl_list, s_compound_stat }, knrStyleFunctionDefinitions);
    ...
    auto forRhs = [&](ForInitKind init, bool clause, bool increment) {
        std::vector<int> rhs { s_for, s_open_paren };
        if (init == ForInitKind::Expression) {
            rhs.push_back(s_exp);
            rhs.push_back(s_semicolon);
        } else if (init == ForInitKind::None) {
            rhs.push_back(s_semicolon);
        } else {
            rhs.push_back(s_decl_for);
        }
        if (clause) {
            rhs.push_back(s_exp);
        }
        ...
        rhs.push_back(s_close_paren);
        rhs.push_back(s_matched);
        return rhs;
    };
    bindForLoop<ForInitKind::Expression, true, true>(registerFor, forRhs);
    bindForLoop<ForInitKind::Expression, true, false>(registerFor, forRhs);
    bindForLoop<ForInitKind::Expression, false, true>(registerFor, forRhs);
    bindForLoop<ForInitKind::Expression, false, false>(registerFor, forRhs);
    bindForLoop<ForInitKind::None, true, true>(registerFor, forRhs);
    bindForLoop<ForInitKind::None, true, false>(registerFor, forRhs);
    bindForLoop<ForInitKind::None, false, true>(registerFor, forRhs);
    bindForLoop<ForInitKind::None, false, false>(registerFor, forRhs);
    bindForLoop<ForInitKind::Declaration, true, true>(registerFor, forRhs);
    bindForLoop<ForInitKind::Declaration, true, false>(registerFor, forRhs);
    bindForLoop<ForInitKind::Declaration, false, true>(registerFor, forRhs);
    bindForLoop<ForInitKind::Declaration, false, false>(registerFor, forRhs);
    ...
    bind(s_struct_declarator, { s_declarator, grammar.symbolId(":"), s_conditional_exp }, [](AbstractSyntaxTreeBuilderContext& context) {
    ...
    bind(s_struct_declarator, { grammar.symbolId(":"), s_conditional_exp }, [](AbstractSyntaxTreeBuilderContext& context) {
                const int width = foldBitFieldWidth(context);
                if (context.failed()) {
                    return;
                }
