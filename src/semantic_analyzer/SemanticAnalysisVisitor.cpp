...
void SemanticAnalysisVisitor::applyPendingArrayMemberBounds() {
    ...
    for (auto& group : pendingArrayMembers().groups()) {
        ...
        for (auto& f : group.members) {
            ...
            if (f.declarator->hasArrayDeclarator()
                    && (folded.isPointer() || type::hasRuntimeSize(folded))) {
                canFold = false;
                break;
            }
        ...
        for (const auto& m : structType.getMembers()) {
        ...
        type::recompleteRecord(structType, std::move(members));
    }
