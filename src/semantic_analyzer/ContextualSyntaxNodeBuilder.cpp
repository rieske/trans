...
void ContextualSyntaxNodeBuilder::functionCall(AbstractSyntaxTreeBuilderContext& context) {
    ...
    context.pushExpression(
            std::unique_ptr<Expression> { new FunctionCall(context.popExpression(), context.popAssignmentExpressionList()) });
}
...
void ContextualSyntaxNodeBuilder::noargFunctionCall(AbstractSyntaxTreeBuilderContext& context) {
    ...
    context.popTerminal(); // (
    context.pushExpression(
            std::unique_ptr<Expression> { new FunctionCall(context.popExpression(), std::unique_ptr<AssignmentExpressionList> {
...
void ContextualSyntaxNodeBuilder::postfixIncrementDecrement(AbstractSyntaxTreeBuilderContext& context) {
    context.pushExpression(std::unique_ptr<Expression> { new PostfixExpression(context.popExpression(), context.popTerminal()) });
}

void ContextualSyntaxNodeBuilder::prefixIncrementDecrement(AbstractSyntaxTreeBuilderContext& context) {
    context.pushExpression(std::unique_ptr<Expression> { new PrefixExpression(context.popTerminal(), context.popExpression()) });
}

void ContextualSyntaxNodeBuilder::unaryExpression(AbstractSyntaxTreeBuilderContext& context) {
    context.pushExpression(std::unique_ptr<Expression> { new UnaryExpression(context.popTerminal(), context.popExpression()) });
}
...
void ContextualSyntaxNodeBuilder::pointerCast(AbstractSyntaxTreeBuilderContext& context) {
    context.popTerminal(); // )
    context.pushExpression(
            std::unique_ptr<Expression> { new PointerCast(context.popTypeSpecifier(), context.popPointer(), context.popExpression()) });
    context.popTerminal(); // (
}
...
void ContextualSyntaxNodeBuilder::parameterDeclaration(AbstractSyntaxTreeBuilderContext& context) {
    context.pushParameter(
            std::unique_ptr<ParameterDeclaration> { new ParameterDeclaration(context.popTypeSpecifier(), context.popDeclaration()) });
}
...
void ContextualSyntaxNodeBuilder::variableDeclaration(AbstractSyntaxTreeBuilderContext& context) {
    context.popTerminal();
    context.pushStatement(
            std::unique_ptr<AbstractSyntaxTreeNode> { new VariableDeclaration(context.popTypeSpecifier(), context.popDeclarationList()) });
}

void ContextualSyntaxNodeBuilder::variableDefinition(AbstractSyntaxTreeBuilderContext& context) {
    context.popTerminal();
    context.popTerminal();
    context.pushStatement(std::unique_ptr<AbstractSyntaxTreeNode> { new VariableDefinition(std::unique_ptr<VariableDeclaration> {
            new VariableDeclaration(context.popTypeSpecifier(), context.popDeclarationList()) }, context.popExpression()) });
}
