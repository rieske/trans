...
void SemanticXmlOutputVisitor::visit(ast::DeclarationSpecifiers& declarationSpecifiers) {
    ...
    for (const ast::StorageSpecifier& storageSpecifier : declarationSpecifiers.getStorageSpecifiers()) {
        ident();
...
void SemanticXmlOutputVisitor::visit(ast::VoidReturnStatement& statement) {
    const std::string nodeId { "return" };
    openXmlNode(nodeId);
    closeXmlNode(nodeId);
}
