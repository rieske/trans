...
struct DataWordSink : init_detail::AggregateInitSink {
    ...
    void storeWord(int offsetBytes, const std::string& operand, int storeSizeBytes) {
        ...
        const int wi = codegen::object_abi::wordIndexAt(offsetBytes);
        if (wi < 0 || wi >= wordCount) {
            return;
        }
        if (storeSizeBytes >= codegen::object_abi::MACHINE_WORD_SIZE) {
        ...
        const int lane = offsetBytes % codegen::object_abi::MACHINE_WORD_SIZE;
...
bool lowerToDataWords(type::Type objectType,
    ...
    const int wordCount = codegen::object_abi::dataWords(totalSize);
    if (wordCount <= 0) {
        return false;
    }
