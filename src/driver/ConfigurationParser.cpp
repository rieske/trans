...
// Options that take an argument and that we intentionally ignore (gcc compatibility).
bool isIgnoredWithArg(const std::string& arg) {
    ...
    return arg == "-MF" || arg == "-MT" || arg == "-MQ" || arg == "-include";
}
...
bool isIgnoredFlag(const std::string& arg) {
    ...
    // -O0, -O1, -O2, -Os, -Og
    if (startsWith(arg, "-O") && arg.size() <= 3) {
        return true;
    }
    if (startsWith(arg, "-W") || startsWith(arg, "-f") || startsWith(arg, "-m")) {
        return true;
    }
    return false;
}
...
void ConfigurationParser::parseOptions(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (isIgnoredFlag(arg) || isIgnoredWithArg(arg)) {
            continue;
        }
