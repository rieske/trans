...
const OptionDef kOptions[] = {
    ...
    { "-a", "-a", OptArgForm::SeparateOrAttached, OptAction::AssemblyDialect },
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (const OptionDef* def = matchOption(arg, attached)) {
            ...
            case OptAction::Resources:
                ...
                resourcesSet = true;
                break;
            case OptAction::AssemblyDialect:
                setAssemblyDialect(value);
                break;
...
std::cerr << " -a <intel|att>\tAssembly dialect (default: intel)" << std::endl;
