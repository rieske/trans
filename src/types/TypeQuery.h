...
// Natural size for packed 1/2/4-byte fields; exact sizeof for aggregates
// (packed 5-byte structs must not write a qword into the next element).
// Otherwise a full qword.
inline int memoryAccessSizeBytes(const Type& t) {
    int size = t.getSize();
    if (size == 1 || size == 2 || size == 4) {
        return size;
    }
    if (t.isAggregate() && size > 0) {
        return size;
    }
    return 8;
}
...
// function-pointer dest; integral 0 into pointers; data pointers accept any
// pointer (product-loose); transparent-union-of-pointers <-> pointer (git sockaddr).
// Expression-sensitive null forms ((void*)0) live in SA productAssignOk
// (semantic_analyzer/ProductAssign.h).
