...
FILE *FiniteAutomatonScanner::logfile = NULL;
...
FiniteAutomatonScanner::FiniteAutomatonScanner(const char *src) {
	if ( NULL == (source = fopen(src, "r"))) {   // nepavyko atidaryti sorso skaitymui
...
int FiniteAutomatonScanner::init() {
	if ( NULL == (lex_src = fopen(lex_source, "r"))) {   // nepavyko atidaryti sorso skaitymui
...
Token FiniteAutomatonScanner::scan(TranslationUnit& translationUnit) {
	...
	do {
		automaton->updateState(lookaheadCharacter);
		if (automaton->isAtFinalState()) {
			return automaton->getCurrentToken();
		}
		lookaheadCharacter = translationUnit.getNextCharacter();
	} while (lookaheadCharacter != '\0');
	throw std::runtime_error("Unable to scan input file");
}

Token* FiniteAutomatonScanner::scan() {
	if (source == NULL)
		return NULL;
	...
	for (EVER) {
		while (buffer[b_index] != EOF)  // kol buferis netuščias, dirbam su juo
		{
			...
			if (current_state == m_state.at(final_state))           // galutinė būsena - turim naują leksemą
				...
				if ((m_state.at(current_state_str)->needsKeywordLookup()) && (m_keywords.find(token) != m_keywords.end()))   // tai keywordas
...
State *FiniteAutomatonScanner::update_state(char c) {
	string next = current_state->nextStateNameForCharacter(c);
