...
```
  ...
  nasm -f elf64
      |
      v
  host gcc -m64 -pie ... + va_tls.o + libc
```
...
| Link | `gcc -m64 -pie` plus objects, `va_tls.o`, and user link args |
...
emit `VaStart`/`VaArg`/`VaCopy`/`VaEnd` instructions; the runtime supplies only the TLS area helpers.
